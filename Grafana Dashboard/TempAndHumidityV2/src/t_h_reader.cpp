...
//      V2 - Final iteration with time
//      V3 - Changed tilt switch to handle push button
//      V4 - Updated WiFi to work better and not hard code IP address
//
//------------------------------------------
// File Dependencies:
//------------------------------------------
...
: dht(sensorPin, DHTTYPE)
{
    ...
    Serial.println("Initializing DHT11 sensor, wait 2 seconds....");
    delay(2000);
...
void TempHumidityReader::checkTiltSwitch() {
    ...
    if (btnPrsd == false && digitalRead(tiltPin) == LOW) {
    ...
    if (btnPrsd == false && digitalRead(tiltPin) == LOW) {
        ...
        Serial.println("Node 2 button press detected!");
        SensorData sensorData = getTempAndHum();
        String dateTime = getDateTime();
